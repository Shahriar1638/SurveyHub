...
router.post('/login', validate(loginSchema), async (req, res) => {
  try {
    ...
    // Server-side verification — email is derived from the verified Firebase ID token,
    // never from the client-supplied body.
    let decoded;
    try {
      decoded = await firebaseAdmin.getAuth().verifyIdToken(idToken);
    } catch (verifyErr) {
    ...
    if (!email) {
      return res.status(401).send({ message: 'No email on Firebase account' });
    }
